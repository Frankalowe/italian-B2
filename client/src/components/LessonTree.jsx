...
export default function LessonTree({ syllabus, progress, toggleUnitProgress, setSelectedPrompt, setView }) {
  ...
  useEffect(() => {
    ...
    async function checkLessonCache() {
      try {
        const res = await fetch(`http://localhost:3001/api/lessons/${selectedUnit.id}/${activeTab}`);
        if (res.ok) {
  ...
  const fetchLessonContent = async () => {
    ...
    setEditedContent('');
    try {
      const res = await fetch('http://localhost:3001/api/ollama/generate-lesson', {
        method: 'POST',
    ...
    <div>
      ...
      {selectedUnit ? (
        <div className="nb-card accent-purple" style={{ marginTop: '2rem' }}>
          ...
          {loadingLesson && (
            <div style={{ padding: '1.5rem', border: '3px solid #000', backgroundColor: '#fff', fontWeight: 800, display: 'flex', gap: '0.5rem', alignItems: 'center', boxShadow: '3px 3px 0px #000' }}>
              <div className="mic-dot listening"></div>
              <span>Tutor is compiling the custom {activeTab} chapter using Gemini...</span>
            </div>
          )}
